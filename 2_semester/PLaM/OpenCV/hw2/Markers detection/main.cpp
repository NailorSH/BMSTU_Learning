...
string pointToString(Point2f point, int level) {
    string result = "{\n";
...
string vectorToString(vector<Point2f> points, int level) {
    string result = "[\n";
    for (int i = 0; i < points.size(); i++) {
        for (int j = 0; j < level + 1; j++) {
            result += "    ";
        }
        result += pointToString(points[i], level + 1);
        if (i < points.size() - 1) {
            result += ",";
        }
        result += "\n";
...
//    cv::aruco::generateImageMarker(dictionary, 24, 200, markerImage, 1);
//    cv::imwrite("marker24.png", markerImage);
    ...
    while(inputVideo.grab()) {
        ...
        for (int i = 0; i < corners.size(); i++) {
            ...
            output += vectorToString(corners[i], 1);
            if (i < corners.size() - 1) {
                output += ",";
            }
            output += "\n";
